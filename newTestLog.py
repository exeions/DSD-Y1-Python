...
def output_summary(name, sn, jr, jd, pjd, ts, jo, ns):
    ...
    with open('job_summaries.txt', 'a') as job_summaries:
        job_summaries.write(f"Engineer Name: {name}\n")
        job_summaries.write(f"Plane Serial Number: {sn}\n")
        job_summaries.write(f"Reason for job: {jr}\n")
        job_summaries.write(f"Date of Maintenance Job: {jd}\n")
        job_summaries.write(f"Date of Previous Job: {pjd}\n")
        job_summaries.write(f"Time spent on job: {ts} hours\n")
        job_summaries.write(f"Job outcome: {jo}\n")
        job_summaries.write(f"Date of next scheduled service: {ns}\n")
